            ...
            <table>
              ...
              <?php while ($row_ch = $result_challenge->fetch_assoc()): ?>
                <tr>
                  ...
                  <?php
                    $stmt = $conn->prepare("SELECT * FROM challenge_attempts WHERE challenge_id = ? AND student_id = ? AND is_correct = 1");
                    $stmt->bind_param("ii", $row_ch['id'], $_SESSION['user']['id']);
                    $stmt->execute();
                    $result_status = $stmt->get_result();
                    $status = ($result_status->num_rows > 0);
                    $stmt->close();
                  ?>
                  <td class="status <?php echo ($status) ? 'correct' : 'incorrect'; ?>">
                    <?php echo ($status) ? 'Done!' : ''; ?>
                  </td>
                </tr>
              <?php endwhile; ?>
            </table>
